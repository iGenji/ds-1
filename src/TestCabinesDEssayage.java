...
public class TestCabinesDEssayage {
	...
	public static void main(String[] args) {
		...
		assertEquals("test3 ko : c.attribuerCabineLibre() : numero cabine : 1, 2 ou 3 et libre", true, test3);
		...
		try{
			...
			assertEquals("test4 ko : : c.attribuerCabineLibre() : numero cabine : 1, 2 ou 3 et libre", true, test4);
		...
		// test 7 : A l'etape precedente, la cabine 1 a ete liberee, c'est la seule cabine libre
		// Cette cabine peut etre reattribuee si elle a bien ete liberee
		try{
			assertEquals("test7 ko : la cabine 1 vient d'etre liberee", 1, c.attribuerCabineLibre());
			assertEquals("test7 ko : nombre cabines libres ko", 0, c.nombreCabinesLibres());
		} catch (Exception e){
		...
		// test 8 : mauvais numero de cabine
		try{
			c.libererCabine(0);
			System.out.println("test8 ko : c.libererCabine(0) : pas de cabine 0 : il fallait une exception");
			System.exit(0);
		} catch (IllegalArgumentException e){		
		}
		
		// test 9 : mauvais numero de cabine
		try{
			c.libererCabine(-1);
		...
		// test 10 : mauvais numero de cabine
		try{
			c.libererCabine(4);
